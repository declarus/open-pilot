...
def get_git_remote():
  try:
    ...
    local_branch = subprocess.check_output(["git", "name-rev", "--name-only", "HEAD"]).strip()  # pylint: disable=unexpected-keyword-arg
    print(local_branch)
    tracking_remote = subprocess.check_output(["git", "config", "branch." + str(local_branch) + ".remote"]).strip()  # pylint: disable=unexpected-keyword-arg
    return subprocess.check_output(["git", "config", "remote." + tracking_remote + ".url"]).strip()  # pylint: disable=unexpected-keyword-arg
  except subprocess.CalledProcessError:
    # Not on a branch, fallback
    return subprocess.check_output(["git", "config", "--get", "remote.origin.url"]).strip()  # pylint: disable=unexpected-keyword-arg
...
try:
  ...
  print(origin)
  if origin.startswith(b'git@github.com:commaai') or origin.startswith(b'https://github.com/commaai'):
